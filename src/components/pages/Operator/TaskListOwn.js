    ...
    const getTaskListByCompany = companyId => {
        ...
        axios.get(config.apiURL + staticDataUrls.task).then(res => {
            ...
            if (res.statusText === "OK") {
                const companyTaskList = res.data.filter(x => x.company_id === companyId);
                companyTaskList.map(x => x.assignedUserIdList.map(i => {
                    if (i === store.user.id) {
                        ownArry.push(x)
                    }
                }));
                ...
                <div className="row">
                    <div className="col-md-12">
                        <div className="page-header">
                            <h1 className="font-weight-bold">Tasks assigned to me</h1>
                        </div>
                    </div>
                </div>
                <hr />
